...
let states : string[]; //Now only string values are allowed
...
function unionTypeFunction(user: string | number) {
    console.log(user);
}
...
result = "SUCCESS";
result = "FAIL";
//result = "anyOtherValue" --> This will give error as only SUCCESS or FAIL is accepted


//Custom types are very useful, If we want to create our own type as -
...
function printAnswer(data: answer) {
    console.log(data);
}
...
function voidReturnType(name: string): void {
...
let anotherFunctionType: Function;
...
let moreSpecificFunctionType: () => boolean;
...
function callBackType(cb: (name: string) => void){
    cb(name);
}
...
let s2: string = vAny //This will work as it is of any type
