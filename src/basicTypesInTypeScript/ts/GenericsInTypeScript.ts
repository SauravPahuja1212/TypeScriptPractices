...
const tab = Array<string>();//Here in angle brackets<> we have delcared type of array which is predefined
...
function anotherAdd<T,U>(a:T, b:U) {
    return Object.assign(a, b);
}
...
function keyOfConstraint<T extends object, U extends keyof T>(a:T, b:U) {
    return a[b];
}
...
class GenericClass<T> {
    private data: T[] = []; //Used above declared generic here
...
const obj2 = new GenericClass<string>();
...
class GenericWithConstraints<T extends object> {}//Now only with object this class can deal